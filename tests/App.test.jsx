...
it('busca por parte do nome sem diferenciar caixa e ignora espaços nas extremidades', async () => {
  const user = open('/user/alice')
  await loaded()
  await user.type(screen.getByRole('searchbox', { name: 'Buscar repositório' }), '  REPO-01  ')
  expect(cards()).toHaveLength(1)
  expect(cards()[0]).toHaveTextContent('repo-01')
  expect(getUserRepositories).toHaveBeenCalledTimes(1)
  expect(getUser).toHaveBeenCalledTimes(1)
})

it('combina busca, linguagem e ordenação antes de paginar e reinicia a página', async () => {
  const user = open('/user/alice')
  await loaded()
  await user.click(next())
  await user.type(screen.getByRole('searchbox'), 'repo-')
  expect(pager()).toHaveTextContent('Página 1 de 3')
  await user.selectOptions(screen.getByLabelText('Linguagem'), 'JavaScript')
  await user.selectOptions(screen.getByLabelText('Ordenar por'), 'name')
  expect(pager()).toHaveTextContent('Página 1 de 2')
  ...
  expect(cards()).toHaveLength(1)
  expect(cards()[0]).toHaveTextContent('repo-12')
  expect(getUserRepositories).toHaveBeenCalledTimes(1)
})

it('anuncia nenhum resultado e recupera a lista ao limpar a busca', async () => {
  const user = open('/user/alice')
  await loaded()
  await user.type(screen.getByRole('searchbox'), 'inexistente')
  expect(screen.queryAllByRole('listitem')).toHaveLength(0)
  expect(screen.getByText(/Nenhum repositório carregado corresponde/)).toHaveAttribute('role', 'status')
  expect(screen.queryByRole('navigation', { name: 'Paginação dos repositórios' })).not.toBeInTheDocument()
  await user.clear(screen.getByRole('searchbox'))
  expect(cards()).toHaveLength(10)
  expect(pager()).toHaveTextContent('Página 1 de 3')
  expect(screen.queryByText(/Nenhum repositório carregado corresponde/)).not.toBeInTheDocument()
  expect(getUserRepositories).toHaveBeenCalledTimes(1)
})

it('aplica a busca existente a novos lotes mesmo quando não havia resultados', async () => {
  getUserRepositories.mockResolvedValueOnce(fullBatch).mockResolvedValueOnce([extraRepository])
  const user = open('/user/alice')
  await loaded()
  await user.type(screen.getByRole('searchbox'), 'EXTRA')
  expect(screen.queryAllByRole('listitem')).toHaveLength(0)
  await user.click(loadMoreButton())
  expect(await screen.findByRole('link', { name: 'extra-repo ↗' })).toBeInTheDocument()
...
it.each(['alice', 'bob'])('limpa a busca por nome ao pesquisar o perfil %s novamente', async (login) => {
  const user = open('/user/alice')
  await loaded()
  await user.type(screen.getByRole('searchbox'), 'repo-01')
  await user.clear(screen.getByRole('textbox', { name: 'Usuário do GitHub' }))
  await user.type(screen.getByRole('textbox', { name: 'Usuário do GitHub' }), `${login}{Enter}`)
  await loaded(login)
